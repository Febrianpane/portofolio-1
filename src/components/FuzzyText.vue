<template>
  <canvas ref="canvasRef" class="fuzzy-canvas" />
</template>
...
export default defineComponent({
  ...
  props: {
    ...
    fontSize: { type: [Number, String], default: 'clamp(2rem, 8vw, 8rem)' },
    fontWeight: { type: [String, Number], default: 900 },
    fontFamily: { type: String, default: 'inherit' },
    color: { type: String, default: '#fff' },
    enableHover: { type: Boolean, default: true },
    baseIntensity: { type: Number, default: 0.18 },
    hoverIntensity: { type: Number, default: 0.5 },
    // Extra outline thickness for stronger color presence
    thickness: { type: Number, default: 1.25 },
  },
  setup(props) {
    ...
    const waitForFont = async (fontFamily, fontWeight, fontSize) => {
      try {
      ...
      } catch (e) { /* noop: font loading API unavailable or failed */ }
      ...
      return new Promise(resolve => {
        ...
        const checkFont = () => {
          ...
          if (newWidth !== testWidth && newWidth > 0) resolve(true)
          else if (attempts++ < 20) setTimeout(checkFont, 50)
          else resolve(false)
        }
    ...
    const initCanvas = async () => {
      ...
      if (testMetrics.width === 0) {
        setTimeout(() => { if (!isCancelled) initCanvas() }, 100)
        return
      }
      ...
      // Stroke to add thickness (outline) using same color
      const lw = Math.max(1, Number(props.thickness) || 1.25)
      offCtx.lineWidth = lw
      offCtx.lineJoin = 'round'
      offCtx.strokeStyle = props.color
